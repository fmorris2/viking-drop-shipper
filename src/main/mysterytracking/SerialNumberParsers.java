...
public class SerialNumberParsers {
    ...
    static class DefaultSerialNumberParser implements SerialNumberParser {
		...
		@Override
        public ArrayList<Integer> apply(String serialNumberString) {
    		final ArrayList<Integer> serialNumber = new ArrayList<>(
            ...
            for (final char c : arr.toCharArray()) {
                serialNumber.add(Character.isDigit(c) ? Character.getNumericValue(c) : ((c) - 3) % 10);
            }
            return serialNumber;
        }
    }
